...
def get_participant_id_from_name(participant_name: str) -> str:
    ...
    # 1. 파일명으로 직접 찾기 (최준혁_connected_group.html 같은 경우)
    try:
        ...
        if output_dir.exists():
            ...
            if html_files:
                for html_file in html_files:
                    # 파일명에서 이름 추출 (예: 최준혁_connected_group.html)
                    file_stem = html_file.stem.replace("_connected_group", "")
                    if file_stem == participant_name:
                        # EB_ 형식이 아니면 그대로 반환 (최준혁)
                        if not file_stem.startswith("EB_"):
                            return file_stem
                        return file_stem
            ...
            if data:
                for key in data.keys():
                    ...
                    if participant_name == extracted_name or participant_name in key:
                        ...
                        # 없으면 키 자체를 반환 (participant_최준혁 -> 최준혁)
                        if key.startswith("participant_"):
                            return key.replace("participant_", "")
                        return key
    except Exception:
        pass
    ...
    if 'participant_id' in st.session_state:
        ...
        if session_id == participant_name or participant_name in session_id:
            return session_id
...
def load_participant_results(participant_id: str):
    ...
    results = {
        ...
        "step3": {
            # participant_name 기반으로 동적 경로 생성됨
            "radar_chart": None,
            "bar_chart": None
        },
        "step4": {
    ...
    # EEG 테이블 찾기 - participant_id가 포함된 파일 우선 검색
    eeg_table_pattern = Path("output/Emotion_EEG/EEG_Tables")
    if eeg_table_pattern.exists():
        # participant_id가 포함된 파일 먼저 찾기
        specific_table = eeg_table_pattern / f"RECORD_{participant_id}_EEG_Table.png"
        if specific_table.exists():
            results["step2"]["eeg_table"] = str(specific_table)
        else:
            # 없으면 모든 파일 중에서 participant_id가 포함된 것 찾기
            table_files = list(eeg_table_pattern.glob("*_EEG_Table.png"))
            for table_file in table_files:
                if participant_id in table_file.name:
                    results["step2"]["eeg_table"] = str(table_file)
                    break
            # 그래도 없으면 첫 번째 파일 사용
            if not results["step2"]["eeg_table"] and table_files:
...
def display_image(path: str, caption: str = None):
    ...
    if path and Path(path).exists():
        ...
        st.markdown(f'<div class="warning-box">파일을 찾을 수 없습니다: {path}</div>', unsafe_allow_html=True)
...
def display_html_file(path: str):
    ...
    if path and Path(path).exists():
        with open(path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        # 빈 HTML 파일인지 확인 (단절된 그룹이 없는 경우)
        if "단절된 키워드 그룹이 없습니다" in html_content or len(html_content.strip()) < 200:
            st.info("단절된 키워드 그룹이 없습니다. 모든 키워드가 메인 감정과 연결되어 있습니다.")
        else:
...
def main():
    ...
    # 세션 상태에서 participant_id 가져오기 (우선순위: 세션 상태 > 기본값)
    session_participant_id = st.session_state.get('participant_id')
    session_participant_name = st.session_state.get('participant_name')
    
    # 세션 상태에 값이 있으면 우선 사용
    if session_participant_id:
        default_participant_id = session_participant_id
        if session_participant_name:
            default_participant_name = session_participant_name
        else:
            default_participant_name = session_participant_id
    else:
        default_participant_id = "EB_002"
        default_participant_name = "김도단"
    
    # 세션 상태의 participant_id가 이름인 경우 (예: "최준혁")
    if default_participant_id and not default_participant_id.startswith("EB_"):
        # 이름 그대로 사용 (파일명이 이름인 경우)
        pass
    else:
        # EB_ 형식인 경우 이름 추출 시도
        extracted_name = get_participant_name_from_eeg_data(default_participant_id)
        if extracted_name:
            default_participant_name = extracted_name
        elif not default_participant_name or default_participant_name == default_participant_id:
    ...
    if st.sidebar.button("결과 불러오기", type="primary", use_container_width=True):
        # 입력한 이름을 세션 상태에 저장
        st.session_state['participant_name'] = participant_name_input
        st.rerun()
    
    participant_id = get_participant_id_from_name(participant_name_input)
    
    # participant_id가 이름인 경우 (EB_ 형식이 아닌 경우) 그대로 사용
    if not participant_id or (participant_id == participant_name_input and not participant_id.startswith("EB_")):
        participant_id = participant_name_input if participant_name_input else default_participant_id
    ...
    # participant_name 설정 - 우선순위: 입력값 > 세션 상태 > EEG 데이터 > participant_id
    if participant_name_input and participant_name_input != default_participant_name:
        participant_name = participant_name_input
    elif session_participant_name:
        participant_name = session_participant_name
    else:
    ...
    # step3 PNG 파일 경로를 participant_name 기반으로 설정
    chart_result_dir = Path("output/Emotion_EEG/Chart_Result")
    radar_chart_path = chart_result_dir / f"{participant_name}_radarchart.png"
    bar_chart_path = chart_result_dir / f"{participant_name}_barchart.png"
    
    # 파일이 존재하는지 확인하고 경로 설정
    if radar_chart_path.exists():
        results["step3"]["radar_chart"] = str(radar_chart_path)
    else:
        # 기존 파일명도 확인 (하위 호환성)
        old_radar_path = chart_result_dir / "radar_chart.png"
        if old_radar_path.exists():
            results["step3"]["radar_chart"] = str(old_radar_path)
    
    if bar_chart_path.exists():
        results["step3"]["bar_chart"] = str(bar_chart_path)
    else:
        # 기존 파일명도 확인 (하위 호환성)
        old_bar_path = chart_result_dir / "bar_chart.png"
        if old_bar_path.exists():
            results["step3"]["bar_chart"] = str(old_bar_path)
    
    # step7 discrepancy PNG 파일 경로를 participant_name 기반으로 설정
    report_images_dir = Path("output/report_images")
    discrepancy_path = report_images_dir / f"{participant_name}_discrepancy.png"
    
    if discrepancy_path.exists():
        results["step7"]["discrepancy"] = str(discrepancy_path)
    else:
        # participant_id 기반 파일도 확인 (하위 호환성)
        old_discrepancy_path = report_images_dir / f"{participant_id}_discrepancy.png"
        if old_discrepancy_path.exists():
            results["step7"]["discrepancy"] = str(old_discrepancy_path)
    
    # step8 final_report MD 파일 경로를 participant_name 기반으로 설정
    final_reports_dir = Path("output/final_reports")
    # final_reports/data 경로 우선 확인
    final_report_data_path = final_reports_dir / "data" / f"{participant_name}_final_report.md"
    final_report_path = final_reports_dir / f"{participant_name}_final_report.md"
    
    if final_report_data_path.exists():
        results["step8"]["final_report"] = str(final_report_data_path)
    elif final_report_path.exists():
        results["step8"]["final_report"] = str(final_report_path)
    else:
        # participant_id 기반 파일도 확인 (하위 호환성)
        old_final_report_data_path = final_reports_dir / "data" / f"{participant_id}_final_report.md"
        old_final_report_path = final_reports_dir / f"{participant_id}_final_report.md"
        if old_final_report_data_path.exists():
            results["step8"]["final_report"] = str(old_final_report_data_path)
        elif old_final_report_path.exists():
            results["step8"]["final_report"] = str(old_final_report_path)
