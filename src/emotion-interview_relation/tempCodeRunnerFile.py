...
class NetworkVisualizerPNG:
    ...
    def create_weighted_circular_layout(self, main_node, connected_nodes, edge_weights):
        """
        가중치 기반 원형 레이아웃
        - 가중치가 클수록 메인 노드에 더 가까이 배치
        - 한 파일 내 가중치 범위에 따라 동적으로 거리 스케일링
        """
        pos = {}
        pos[main_node] = np.array([0.0, 0.0])  # 메인 노드 중앙
        ...
        # 이 그래프에서 실제 등장하는 가중치들
        weights = [edge_weights.get((main_node, node), 0.0) for node in connected_nodes]
        w_min = min(weights)
        w_max = max(weights)
        ...
        for i, node in enumerate(connected_nodes):
            w = edge_weights.get((main_node, node), w_min)

            if abs(w_max - w_min) < 1e-6:
                # 가중치가 모두 비슷하면 중간 거리로
                radius = (inner_r + outer_r) / 2.0
            else:
                # 0~1로 정규화 후, 강한 연결일수록 radius가 작도록
                norm = (w - w_min) / (w_max - w_min)
                radius = outer_r - norm * (outer_r - inner_r)
    ...
    def visualize_single_file(self, filename: str):
        ...
        # inter_node_relations 기준으로 연결/단절 분리
        connected_to_main = []
        ...
        if connected_to_main:
            ...
            ax.axis('off')
            ax.set_xlim(-8, 8)   # 거리 차이 잘 보이게 약간 확대
            ax.set_ylim(-8, 8)
        ...
        if disconnected_from_main:
            ...
            if 'intra_node_relations' in data:
                for relation in data['intra_node_relations']:
                    if (
                        relation['word1'] in disconnected_from_main and 
            ...
            # spring 레이아웃: k를 줄여서 모이게 하고, 이후 스케일로 한 번 더 중앙으로 압축
            ...
            ax.axis('off')
            ax.set_xlim(-4, 4)   # 중앙 근처에 모이도록 축도 좁게
            ax.set_ylim(-4, 4)
